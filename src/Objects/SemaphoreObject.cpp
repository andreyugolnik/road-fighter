...
bool CSemaphoreObject::cycle(unsigned char* keyboard, unsigned char* old_keyboard)
{
    ...
    if (timmer >= SEMAPHORE_TIME * 1)
        tile = 1;
    if (timmer >= SEMAPHORE_TIME * 2)
        tile = 0;
    if (timmer >= SEMAPHORE_TIME * 3)
        tile = 2;
    if (timmer >= SEMAPHORE_TIME * 4)
        tile = 0;
    if (timmer >= SEMAPHORE_TIME * 5)
        tile = 3;
    if (timmer >= SEMAPHORE_TIME * 6)
        tile = 0;
    if (timmer >= SEMAPHORE_TIME * 7)
        tile = 4;

    /* sound: */
    if (timmer == SEMAPHORE_TIME * 1)
        if (game->S_redlight != 0)
            Sound_play(game->S_redlight);
    if (timmer == SEMAPHORE_TIME * 3)
        if (game->S_redlight != 0)
            Sound_play(game->S_redlight);
    if (timmer == SEMAPHORE_TIME * 5)
