...
CPlayerCarObject::CPlayerCarObject(int nx, int ny, List<CTile>* l, int first_tile, int last_tile, int lk, int rk, int fk, int sc, int init_delay, CGame* g)
{
    ...
    for (i = 0; i < (last_tile - first_tile) + 1; i++)
...
bool CPlayerCarObject::cycle(unsigned char* keyboard, unsigned char* old_keyboard)
{
    ...
    case 1:
        ...
        if (y > -32)
        {
            ...
            if (f < 0.1)
                ...
                f = (((f - 0.1F) / 0.9F) * 0.8F) + 0.2F;

            if (keyboard[left_key])
            {
                if (!keyboard[right_key] || !old_keyboard[left_key] || (x_speed < 0 && !old_keyboard[right_key]))
                {
            ...
            {
                if (!keyboard[left_key] || !old_keyboard[right_key] || (x_speed > 0 && !old_keyboard[left_key]))
                {
                    x_speed = int(MAX_HSPEED * f);
                }
            }
    ...
    case 4:
        ...
        if (state_timmer >= 75)
        {
            ...
            if (!game->object_collision(8, 0, this, CONSTITUTION_SOLID))
            {
                int i, j;
                bool found = false;
    ...
    sound_timmer++;
    if (fuel < int(0.15F * MAX_FUEL) && fuel > 0 && (sound_timmer % 45) == 0)
    {
        if (game->S_fuelempty != 0)
            Sound_play(game->S_fuelempty);
    }
...
*/
                ...
                if (state == 5 || state == 6)
                {
                    ...
                    if (game->S_carskid != nullptr)
                    {
                        ip = (Sint16*)S_carskid_working.abuf;
                        ip2 = (Sint16*)game->S_carskid->abuf;
                        for (i = 0, j = 0; i < game->S_carskid->alen && ((unsigned int)(j * f)) * 4 < game->S_carskid->alen; i += 4, j++)
                        {
                            k = (unsigned int)(j * f);
                            ip[j * 2] = ip2[k * 2];         /* L */
                            ip[j * 2 + 1] = ip2[k * 2 + 1]; /* R */
    ...
    if (state != 4 && state != 0)
    {
        ...
        if (o != 0)
        {
            ...
            if (next_bonus == 300)
                next_bonus = 500;
            if (game->S_takefuel != 0)
                Sound_play(game->S_takefuel);
        }
        ...
        if (o != 0)
        {
            ...
            if ((-y_speed) > 0.75F * MAX_SPEED || o->constitution_test(CONSTITUTION_CAR))
            {
                if (fuel > FUEL_LOSS * 2)
                    fuel -= FUEL_LOSS;
                state = 4;
            ...
            else
            {
                if (game->object_collision(8, 0, this, CONSTITUTION_SOLID) != 0)
                {
                    state_timmer = 0;
                    state = 3;
                    if (game->S_collision != 0)
                        Sound_play(game->S_collision);
                }
                else
                {
                    if (game->object_collision(-8, 0, this, CONSTITUTION_SOLID) != 0)
        ...
        else
        {
            ...
            if (o != 0)
            {
                ...
                if (last_collision == 0 && game->S_collision != 0)
                    Sound_play(game->S_collision);
...
void CPlayerCarObject::draw(int sx, int sy, SDL_Surface* screen)
{
    ...
    if (tile >= 0 && tile < ntiles)
    {
        if (blinking_time > 0)
        {
            int f = (int)(sinf(blinking_time * 0.5f) * 50.0f + 50.0f);
...
{
    const int x1v[8] = { 6, 16, 23, 27, 21, 16, 8, 5 };
    const int y1v[8] = { 7, 3, 8, 16, 25, 25, 22, 12 };
    const int x2v[8] = { 21, 27, 23, 16, 6, 5, 8, 16 };
    const int y2v[8] = { 7, 12, 22, 25, 25, 16, 8, 3 };
    long nt = ntiles - explosion_tiles;
    float tmp;
    int tile, tile2;
