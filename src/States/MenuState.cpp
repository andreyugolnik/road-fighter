    ...
    case 2:
        if (menu_redefining_key != -1)
        {
            ...
            for (i = 0; !found && i < SDLK_LAST; i++)
            {
                ...
                {
                    if ((SDLKey)i != SDLK_ESCAPE && (SDLKey)i != SDLK_F12)
                    {
                        switch (menu_redefining_key)
                        {
                        case 0:
                            if (i != left_key && i != fire_key)
                                right_key = (SDLKey)i;
                            break;
                        case 1:
                            if (i != right_key && i != fire_key)
                                left_key = (SDLKey)i;
                            break;
                        case 2:
                            if (i != right_key && i != left_key)
                                fire_key = (SDLKey)i;
                            break;

                        case 3:
                            if (i != left2_key && i != fire2_key)
                                right2_key = (SDLKey)i;
                            break;
                        case 4:
                            if (i != right2_key && i != fire2_key)
                                left2_key = (SDLKey)i;
                            break;
                        case 5:
                            if (i != right2_key && i != left2_key)
                                fire2_key = (SDLKey)i;
                            break;
                        }
                    }
        ...
        else
        {
            ...
            if (menu_item < 0)
                menu_item = 0;
            if (menu_item > menu_nitems - 1)
                menu_item = menu_nitems - 1;

            if ((keyboard[fire_key] && !old_keyboard[fire_key]) || (keyboard[SDLK_SPACE] && !old_keyboard[SDLK_SPACE]) || (keyboard[right_key] && !old_keyboard[right_key]) || (keyboard[left_key] && !old_keyboard[left_key]) || (keyboard[SDLK_ESCAPE] && !old_keyboard[SDLK_ESCAPE]))
            {
                if ((keyboard[left_key] && !old_keyboard[left_key]) || (keyboard[SDLK_ESCAPE] && !old_keyboard[SDLK_ESCAPE]))
                {
                    menu_item = menu_nitems - 1;
                }
                ...
                case 2:
                    if (menu_item == 0)
                        menu_redefining_key = 0;
                    if (menu_item == 1)
                        menu_redefining_key = 1;
                    if (menu_item == 2)
                        menu_redefining_key = 2;
                ...
                case 3:
                    if (menu_item == 0)
                        menu_redefining_key = 3;
                    if (menu_item == 1)
                        menu_redefining_key = 4;
                    if (menu_item == 2)
                        menu_redefining_key = 5;
