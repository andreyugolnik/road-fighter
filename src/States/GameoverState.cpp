...
int CRoadFighter::gameover_cycle()
{
    ...
    if (gameover_state == 0 && (state_timmer >= gameover_time * 6 || (state_timmer >= gameover_time && ((keyboard[fire_key] && !old_keyboard[fire_key]) || (keyboard[SDLK_ESCAPE] && !old_keyboard[SDLK_ESCAPE])))))
    {
        gameover_state = 1;
        if (gameover_timmer >= gameover_time)
            gameover_timmer = gameover_time;
    }
...
void CRoadFighter::gameover_draw(SDL_Surface* screen)
{
    ...
    if (gameover_state == 1)
    {
        if (scoreboard_fade_timmer > 0)
            scoreboard_fade_timmer--;
        if (scoreboard_x < screen->w)
            scoreboard_x += int((screen->w - desired_scoreboard_x) / gameover_time);
        if (scoreboard_x > screen->w)
